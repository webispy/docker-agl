...
ENV AGL_BASE=/opt/agl-sdk/11.0.4-aarch64 \
...
RUN apt-get update \
	...
	&& wget --no-verbose https://download.automotivelinux.org/AGL/release/koi/latest/raspberrypi4/deploy/sdk/poky-agl-glibc-x86_64-agl-demo-platform-crosssdk-aarch64-raspberrypi4-64-toolchain-11.0.4.sh \
	&& chmod +x *.sh \
	&& ./poky*.sh \
	&& rm *.sh \
	&& apt-get clean \
